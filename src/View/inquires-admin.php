...
<div class="container py-5">
    ...
    <?php foreach($inquires as $inquire):?>
        <div class="t-row" data-toggle="modal" data-target="#view-modal-<?=$inquire->id?>">
            ...
            <?php if($inquire->answered == false):?>
                <div class="cell-10"><button class="btn-filled" data-toggle="modal" data-target="#insert-modal-<?=$inquire->id?>">답변하기</button></div>
            <?php endif;?>
        </div>
        ...
        <form action="/insert/answers" method="post" id="insert-modal-<?=$inquire->id?>" class="fade modal">
